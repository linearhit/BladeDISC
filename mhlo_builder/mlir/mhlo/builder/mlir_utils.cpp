...
mlir::Value BuildStandardI32NumelOfTensor(
  ...
  for (mlir_dim_t dim : numel_dims) {
    ...
    dim_size = builder.create<mlir::arith::IndexCastOp>(loc, dim_size, mhlo_dim_type);
...
mlir::Value BuildResolveUnknownDimSizeI32(mlir::OpBuilder& builder,
  ...
  for (mlir_dim_t k = 0; k < output_rank; ++k) {
    ...
    reduce_prod = builder.create<mlir::arith::MulIOp>(loc, dim_size, reduce_prod);
    shape_elements.push_back(dim_size);
  }

  for (mlir_dim_t k = 0; k < output_rank; ++k) {
    auto dim_size = shape_elements[k];
    auto is_minus_1 = builder.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::eq,
                                                   dim_size, minus_1);
