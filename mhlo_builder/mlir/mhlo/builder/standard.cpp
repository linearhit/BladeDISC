...
mlir::Value BuildStdConstForBool(mlir::OpBuilder& builder,
  ...
  return builder.create<mlir::arith::ConstantOp>(loc, builder.getBoolAttr(value));
}
...
mlir::Value BuildStdConstForF64(mlir::OpBuilder& builder,
  ...
  return builder.create<mlir::arith::ConstantOp>(loc, builder.getF64FloatAttr(value));
}
...
mlir::Value BuildStdMaximumSigned(mlir::OpBuilder& builder,
                                  ...
                                  const mlir::Value& std_lhs,
                                  const mlir::Value& std_rhs) {
  return BuildStdSelectSigned(builder, loc, mlir::arith::CmpIPredicate::sge, std_lhs,
                              std_rhs);
}
...
mlir::Value BuildStdMinimumSigned(mlir::OpBuilder& builder,
                                  ...
                                  const mlir::Value& std_lhs,
                                  const mlir::Value& std_rhs) {
  return BuildStdSelectSigned(builder, loc, mlir::arith::CmpIPredicate::sle, std_lhs,
                              std_rhs);
}
...
llvm::Optional<mlir::Value> BuildCastStdConstScalarToHloConstTensor(
  ...
  auto def = llvm::dyn_cast<mlir::arith::ConstantOp>(std_scalar.getDefiningOp());
  if (!def) {
    return llvm::None;
  }
...
mlir::Value BuildStdScalarToHloTensor(mlir::OpBuilder& builder,
  ...
  mlir::Value hlo_tensor =
      builder.create<mlir::tensor::FromElementsOp>(loc, dim_sizes);
  ...
  for (auto dsize : dim_sizes) {
    ...
    if (dsize_type != mhlo_dim_type) {
      if (!dsize_type.isIndex()) {
        dsize =
            builder
                .create<mlir::arith::IndexCastOp>(loc, dsize, builder.getIndexType())
                .getResult();
      }
      dsize = builder.create<mlir::arith::IndexCastOp>(loc, dsize, mhlo_dim_type)
                  .getResult();
    }
