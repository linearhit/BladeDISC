...
mlir::Value BuildStdNormalizeIndex(mlir::OpBuilder& builder,
  ...
  auto not_zero = builder.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::ne,
                                               dim_size, zero);
  ...
  // cond = index >= dim_size
  auto cond = builder.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::sge, index,
                                           dim_size);
...
mlir::Value BuildDynamicSliceInternal(mlir::OpBuilder& builder,
  ...
  for (mlir_dim_t r = 0; r < rank; ++r) {
    if (r == dim_index) {
      ...
      auto mhlo_end_index =
          builder.create<mlir::arith::IndexCastOp>(loc, norm_end_index, mhlo_dim_type);
