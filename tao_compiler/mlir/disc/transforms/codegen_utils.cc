...
std::pair<ParallelOp, ParallelOp> tileParallelLoop(ParallelOp op,
  ...
  for (size_t i = 0, end = op.getUpperBound().size(); i != end; ++i) {
    if (i < tileSizes.size())
      ...
      tileSizeConstants.push_back(b.create<arith::ConstantIndexOp>(op.getLoc(), 1));
  }
  ...
  for (auto step : llvm::zip(op.getStep(), tileSizeConstants)) {
    newSteps.push_back(
        b.create<arith::MulIOp>(op.getLoc(), std::get<0>(step), std::get<1>(step)));
  }
  ...
  for (auto dim : llvm::zip(outerLoop.getLowerBound(), outerLoop.getUpperBound(),
    ...
    auto tileSize =
        cast<arith::ConstantIndexOp>(tileSizeConstant.getDefiningOp()).getValue().cast<IntegerAttr>().getInt();
  ...
  if (withInboundCheck && needInboundCheck) {
    ...
    for (auto dim :
      ...
      Value index = b.create<arith::AddIOp>(
          ...
          outerIV);
      Value dimInbound = b.create<arith::CmpIOp>(op.getLoc(), arith::CmpIPredicate::ult,
    ...
    for (auto ivs : llvm::enumerate(llvm::zip(innerLoop.getInductionVars(),
      ...
      arith::AddIOp newIndex = b.create<arith::AddIOp>(op.getLoc(), std::get<0>(ivs.value()),
                                         std::get<1>(ivs.value()));
    ...
    for (auto ivs : llvm::zip(innerLoop.getInductionVars(),
      ...
      arith::AddIOp newIndex =
          b.create<arith::AddIOp>(op.getLoc(), std::get<0>(ivs), std::get<1>(ivs));
