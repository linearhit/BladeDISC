...
static bool isSinkingBeneficiary(Operation* op) {
  return isa<ConstantOp, memref::DimOp, SelectOp, arith::CmpIOp>(op);
}
...
LogicalResult rewriteLaunchOpSetting(scf::ParallelOp parallelOp,
  ...
  for (auto&& en :
       llvm::enumerate(llvm::zip(parallelOp.getLowerBound(),
  ...
  for (int i = 0; i < numIvs; ++i) {
    ...
    Value stepIsOne =
        builder.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, stepVec.back(), one);
  ...
  if (failed(cloneAndMoveTo(builder, parallelOp, lowerBoundVec, upperBoundVec,
                            stepVec, &ifOp.getElseRegion().getBlocks().front()))) {
    return ifOp->emitError("failed to cloned the parallel op\n");
  }
  parallelOp->erase();
  return success();
