...
LogicalResult ComputeReshapeShapeOpConverter::matchAndRewrite(
  ...
  Value isZero =
      rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, numElements, zero);
  Value isNeg =
      rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, accumNumElems, zero);
  ...
  for (int64_t i = 0; i < rank; ++i) {
    ...
    Value isNegOne = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq,
    ...
    if (!targetElemType.isIndex())
      newShapeDimValue =
          rewriter.create<arith::IndexCastOp>(loc, newShapeDimValue, targetElemType);
