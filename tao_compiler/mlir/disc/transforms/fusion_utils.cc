...
scf::ParallelOp StitchCPUAnalysis::emitTileParallelLoop(OpBuilder& b,
  ...
  for (int axis : parallelAxes) {
    ...
    steps.push_back(
        b.create<arith::ConstantIndexOp>(loc, indexStore[info.indices[axis]].step));
  }
...
Value allIndicesZeros(OpBuilder& b, Location loc, ValueRange indices) {
  ...
  for (Value idx : indices) {
    Value indexIsZero = b.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, idx, zero);
...
bool StitchCPUAnalysis::emitBcastOpParallelIndex(OpBuilder& b, Location loc,
  ...
  for (int d = 0; d < rank; ++d) {
    ...
    if (it != from.indices.end()) {
      ...
      Value isEqual = b.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, inDimValue,
                                             outDimValue);
      Value zero = b.create<arith::ConstantIndexOp>(loc, 0);
      ...
      Value isZero = b.create<arith::CmpIOp>(
          loc, arith::CmpIPredicate::eq, from.symbolIndices[outParallelIdx], zero);
